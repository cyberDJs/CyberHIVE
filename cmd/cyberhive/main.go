...
func run(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	case "fetch":
		...
		return fetchArtifact(ctx, args[1], args[2], args[3], args[4], httptransport.NewClient(30*time.Second), stdout)
	case "secure-fetch":
		if len(args) != 8 {
...
func fetchArtifact(ctx context.Context, manifestPath, peersPath, casDir, output string, client swarm.ChunkClient, stdout io.Writer) error {
	...
	fetcher, err := swarm.NewFetcher(store, inventory, client, 4)
	if err != nil {
		return err
	}
