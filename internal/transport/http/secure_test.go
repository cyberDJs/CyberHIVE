...
func TestSecureSwarmAllowsAuthorizedPeerAndRejectsUnauthorizedPeer(t *testing.T) {
	root := t.TempDir()
	...
	if _, err := unauthorizedClient.Fetch(context.Background(), server.URL, m.SHA256, m.Chunks[0].SHA256); err == nil || !strings.Contains(err.Error(), "403") {
